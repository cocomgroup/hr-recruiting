...
interface JobsState {
  ...
  filters: {
    department: string;
    employmentType: string; // Changed from 'type' to 'employmentType'
    location: string;
    search: string;
  };
}
...
export const filteredJobs = derived(jobsStore, $jobsStore => {
  ...
  if ($jobsStore.filters.employmentType) {
    filtered = filtered.filter(job => 
      job.employmentType.toLowerCase() === $jobsStore.filters.employmentType.toLowerCase()
    );
  }
...
export const departments = derived(jobsStore, $jobsStore => {
  const depts = new Set($jobsStore.jobs.map(job => job.department).filter(Boolean));
  return Array.from(depts).sort();
});

export const locations = derived(jobsStore, $jobsStore => {
  const locs = new Set($jobsStore.jobs.map(job => job.location).filter(Boolean));
  return Array.from(locs).sort();
});

export const jobTypes = derived(jobsStore, $jobsStore => {
  const types = new Set($jobsStore.jobs.map(job => job.employmentType).filter(Boolean));
